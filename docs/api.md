...
- Notes：每项为“最新记录快照”，匹配 `Indicators.vue` 表格字段；数据来源于数据库（`Indicator`/`IndicatorRecord`/`Category`/`UserIndicator`），不再使用测试数据；`favorite` 字段来源于 `UserIndicator`；`isBuiltin/loinc` 用于区分系统内置指标与自定义指标；`indicator/nameCn/nameEn` 映射 `Indicator.name_cn/name_en`。
...
- Endpoint：`/indicators`
- Method：`POST`
- Request（Body）：`{ "nameCn": "血压", "nameEn": "Blood Pressure", "type": "numeric|text", "unit": "mmHg", "referenceMin": 90, "referenceMax": 140, "categories": ["血压"], "loinc": null }`
...
- Method：`GET`
- Response（Body）：`{ "id": 101, "nameCn": "血压", "nameEn": "Blood Pressure", "type": "numeric|text", "unit": "mmHg", "referenceMin": 90, "referenceMax": 140, "isBuiltin": true, "loinc": "BP_SYS", "categories": ["血压","心血管"] }`
...
- Response（Body）：`{ "items": [ { "id": 1, "name": "血压", "description": "..." } ], "total": 12 }`
- Notes：数据源为数据库表 `Category`。
...
- Method：`GET`
- Response（Body）：`{ "id": 1, "name": "血压", "description": "...", "indicatorCount": 24 }`
- Notes：`indicatorCount` 为分类成员数量，来源于 `IndicatorCategoryLink`。

### 分类下的指标列表（Get Indicators by Category）

- Prefix：`/api/v1`
- Endpoint：`/categories/{id}/indicators`
- Method：`GET`
- Request（Query）：`page`, `pageSize`, `keyword`, `favorites=true|false`
- Response（Body）：与 `GET /indicators` 的 `items` 结构一致。
- Notes：按分类过滤并返回“最新记录快照”。
...
{
  "series": [
    { "date": "YYYY-MM-DD", "value": 120, "refLow": 90, "refHigh": 140 }
  ],
  "stats": {
    "mean": 115.2,
    "std": 8.3,
...
axios.get("/api/v1/indicators", {
  params: { page: 1, pageSize: 20, keyword: "血", category: "血压" },
});
```
...
```js
axios.get("/api/v1/indicators", {
  params: { page: 1, pageSize: 20, favorites: true },
});
```
...
```js
axios.get("/api/v1/admissions/adm-1001/files/f-1/indicator-records", {
...
```js
axios.post("/api/v1/admissions/adm-1001/files/f-1/ocr", {
  language: "zh",
  engine: "paddleocr",
});
```
...
```json
{
  ...
  "data": {
    ...
    "dashboard": {
      "keyIndicators": [
        { "id": 101, "name": "收缩压", "unit": "mmHg", "current": 118 }
      ],
      "abnormalCount": 3,
      "recentChanges": []
    },
